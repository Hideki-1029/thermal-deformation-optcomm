...
Sentinel-1（dawn-dusk SSO、SEIRIOS/LTAN06 条件の代理）を使い、POD を真値として軌道予測誤差を PAT 非熱項に載せる。
...
- 「最新 TLE を過去へ backward 伝搬」は運用と一致せず、誤差を過大評価する。**必ず epoch ≤ 評価時刻 の TLE を forward 伝搬**する。
- 300 urad は最悪側ではない（TLE age 7 日なら ~600 urad 程度）。「保守寄り」とは** GNSS を載せない前提を取る**という研究上の意味。

実装：`src/orbit/run_orbit_prediction_error.py`、PAT 連携は `pat_femap_los_config.yaml` の `orbit_error.source: sentinel1_tle_vs_pod`。データ手順は `data/orbit/README.md`。
...
1. **TLE-only** → 軌道 ~300 urad、熱ひずみと同程度 → 両方共存の中での熱補正評価
2. **GNSS / RESORB 級** → 軌道誤差小 → 熱補正の効果が相対的に強調される（SEIRIOS 向け）

五十里先生指摘（軌道予測誤差に注意）への答え：TLE-only でも熱ひずみと同桁になり得ることを実データで示し、その上で熱補正を評価する。GNSS 搭載は改善オプションとして別論。
