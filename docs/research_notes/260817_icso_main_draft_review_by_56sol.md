...
### 2. 「学習に用いなかったケース」は少し強すぎる→実装を変更。aもLOCOする
...
## 2026-08-18 文書修正・圧縮（第2パス）

上記方針と対応完了メモを維持したまま、未対応だった原稿上の修正を実施した。

### 圧縮

- abstractを1,169文字から約538文字へ短縮し、主結果と予備的残差更新の位置づけを分離した。
- 序論の研究質問・貢献・適用範囲を1段落へ統合した。
- 関連研究の4小節と2比較表を、2段落と1比較表へ統合した。JANUSとの差分は俯瞰表後の本文に残した。
- 21行のケース行列表を、検証目的別の7グループへ集約した。
- 残差更新を独立章からPAT章内の「予備的な残差更新」へ移し、重複していた比較図を削除した。比較表と主要な前提・限界は維持した。
- 議論・限界・結論の反復を4段落へ統合した。
- 図表ブロックは18個から16個、PDFは21ページから15ページへ削減した。

### その他の本文修正

- 所属に本郷のstreet addressとpostal codeを追加した。
- 熱解析の初期温度20 °C、周期定常化後の出力、TD Mapperによる温度写像、STT近傍の3--2--1拘束を解析条件へ追記した。
- パネル中心温度センサを想定し、温度差誤差0.1 °Cが約3 µradに相当する感度を考察へ追記した。
- 半電力ケースを「外挿」ではなく、ON/OFF二値入力が連続発熱量を表現できないモデル仕様誤差として記述した。
- 本文中の内部実装名 `bcase` を論文用の「階層温度差モデル」へ変更した。
- Zhang/Cheng文献、Rüddenklau文献、所属、誤記を修正した。

### 確認

- `typst compile --root . papers/icso/main.typ papers/icso/main.pdf` 成功。
- 生成PDFは15ページ。
- `git diff --check` で空白エラーなし。

### 今回は変更していない項目

- 方針どおり、ケース内static biasは比較表へ追加していない。
- 成分アブレーション、捕捉失敗を含む指標、複数seed評価は追加していない。
- 走査外周の厳密値と走査中誤差の時間変化は、方針どおり記述を変更していない。
- 熱・構造メッシュ総数は、リポジトリ内で論文に記載できる確定値を確認できなかったため追加していない。
- 英語化と最終SPIE整形は未実施。
