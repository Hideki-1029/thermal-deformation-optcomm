...
- **2026-08-16 追記：このノートの 12 章構成は「ページ数を制限しない詳細版」（修士論文の章の素案）として位置づけを変更。** ICSO 2026 提出用のフルペーパーは、締切（8/20）と読みやすさ（JANUS STOP 論文など 12 p 級が標準）を考慮し、12–16 p のコンパクト版として別途構成する。ICSO 版の章立ての正本は [`260816_icso_compact_paper_outline.md`](260816_icso_compact_paper_outline.md) を参照。本ノートの詳細な記述内容（JANUS の読み、観察、モデル選択の根拠、limitations 等）は ICSO 版でも中身の参照先として有効。
