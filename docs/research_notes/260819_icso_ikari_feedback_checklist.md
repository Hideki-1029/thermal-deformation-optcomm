...
- [ ] p. 2　`making it potentially one of the largest initial pointing-error components. This study`
...
- [ ] p. 1　`the mean subsequent-orbit RMSE on the dominant axis was 5.5 µrad, compared with a median raw LOS RMS of 615 µrad.`
  - FB: 「mean RMSE と median RMS を並べていて若干不自然？」

### 姿勢抽出方法

- [ ] p. 6　`The relative LOS time series is extracted from the rotations of the representative STT and LCT nodes at their centers.`
...
- [ ] p. 8　`Four sun faces; PROP only, PCDU only, or no additional dissipation`
...
- [ ] p. 13　`COLD/baseline-surface cases, Cases 4–6 and 8–21`
  - FB: 「ケースの番号の対応は合っていますか？」

### 図と本文の数値整合

- [ ] p. 11　`The dominant-axis bias RMSE is approximately 3.1 µrad in-sample and 3.8 µrad`
  - FB: 「図中の記載数値2.1、2.3 µrad と値が食い違っている？」

- [ ] p. 12　`In the PY all-dissipation case, for example, the raw RMS was approximately 1250 µrad and the post-model RMSE approximately 4 µrad.`
...
- [ ] 高嶋さんの9件の対応後、ケースID、本文数値、図中数値、PAT試行条件の相互整合を再監査する。
- [ ] `main_en.typ` を再ビルドし、修正後PDFの図表と改ページを確認する。
...
- [ ] p. 4　`Acquisition evaluation`
  - FB: 「全体的に、表内文の改行位置をもう少し見やすいように直した方が良さそうです。」

- [ ] p. 7　`In the deformation budget of a representative case, the mean centerline tilt between the STT and LCT reference points`
  - FB: 「公式フォーマット通りなら問題ないが、表前後の行間が全体的に少ないように感じます。確認してみてください。」

### 熱ひずみに注目する論理

- [ ] p. 2　`Second, it is governed by known operating conditions, including eclipse cycles, the sun-facing panel, surface optical properties, and internal dissipation, and is therefore not a wholly unknown disturbance.`
...
- [ ] p. 12　`7.1. Scan conditions`
  - FB: 「紙面に余裕があるならば、scan軌跡の図を入れると分かりやそうです」

### 捕捉後残差の観測可能性

- [ ] p. 14　`the post-acquisition dominant-axis residual`
  - FB: 「以前RGで議論したことにも関連しますが、捕捉完了後の熱ひずみによる残差はどのように観測するのでしょうか。本節の目的としては、そこは考えなくて良いんですかね。」

## 草野さんコメントの対応後確認

- [ ] 草野さんの8件の対応後、題名と表内の改行、表前後の間隔、FFの初出、序論の論理展開を再確認する。
- [ ] 残差更新節で、捕捉後残差の観測可能性と本節の評価範囲が明確か確認する。
