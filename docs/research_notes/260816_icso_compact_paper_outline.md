...
#### 序論
- 「特に粗捕捉段階では...」：
  - 粗捕捉と言ったときにカバー範囲が広く、精追尾のQD-FPM系までCCDによって追い込むことも粗捕捉ということがあるので、初期捕捉とかが表現としては適切？
- 「本研究が着目するのは、低軌道 (Low Earth Orbit; LEO) 衛星の熱変形に起因する指向バイアスである」：
  - ここの文、ないしその後にそれ以前に説明として存在していた「初期指向誤差」のwordを入れる方が分かりやすそう。つまり、熱ひずみによる指向バイアスが初期指向誤差の一部を占めることを説明したい。
...
- 「この相対姿勢変化は、粗捕捉開始時にはまだ光学フィードバックで補正されていないため、scan center のずれとして現れる」：
  - scan centerが唐突。粗捕捉開始時の...とかつけるといいかも
- 「従来のPAT検討では、初期指向誤差をまとめて確率的な不確かさとして扱うことが多い」：
  - これはそうなの？不確かさとして扱った上で、スキャンやビーコン光の拡がり角の設計で不確かさを吸収するイメージだった。
- 「本稿の中心的な立場は、観測された指向残差から熱変形成分だけを完全に分離することではなく、」：
  - それはそうなんだけど、ここでそれをいきなり出すと、読者はなぜいきなり観測の残差から分離しようとする話しが出たのか疑問に思うかも
- 「なお、熱LOSと軌道予測誤差は同じ軌道周期帯の周波数成分を持つため、観測後の単純な周波数分離で両者を切り分けることは困難である。だからこそ、温度と運用状態に基づく事前の feedforward 補正に意味がある。」：
  - これも軌道予測誤差と分離する話しがいきなりで唐突かな。確か、初期指向誤差に含まれる様々な誤差の中で、最もサイズが大きいのが軌道予測誤差と熱ひずみ誤差？（：これは解析して出た値を根拠にするか、参考文献を引っ張るか）で、しかもその2つは周波数が近いから分離しづらいという文脈があった気がする（がその文脈がこの序論ではうまく説明できていない気がする）
- 「。JANUS 光学ヘッドでは、構造の」：
  - うーん。「JANUS光学ヘッド」という略称でうまく伝わるかな。アブストじゃないし、もう少し丁寧でもいいかも？
  - あーでも次の章で先行研究詳しくやるのか。ならいいか。
- 「しかし、衛星バス上に離れて搭載されたSTTとLCTの相対LOSを対象として、その軌道内の時変成分とケース間のバイアスを分離して軽量に予測し、光通信粗捕捉の scan center 補正へ接続した枠組みは十分に整理されていない」：
  - ここもう少し一般的な表現にして強くしちゃだめ？今の文だとかなりニッチなことをやっているように見える。例えば「衛星バス上で発生した熱ひずみ」
  ...
  - ★第一修正後に思ったこと
    - やっぱ「JANUS光学ヘッドは、」は気になるな。もっとわかりやすい表現にするか、説明をもう少し付けて
    - 先行研究の章でもいいかもだけど、JANUSとの違いとして、今回は光通信の初期捕捉への適用というのもありそう（いやでもこれは第二章以降でいいか）
    - どちらかというと、序論で推したい差分としては、「特定の軌道・熱環境ではなく、一般的な衛星の軌道やモードに対してもできる」ってことなのかな。ちょっと序論で推したい差分を整理したいね
      - まあいったん今のままでええか

#####   次章以降への適用ルール（序論FBから抽出、260816）

- 数値の出所を分離する: 文献値には引用、自前の解析値は「本研究の解析では」等で明示。文献にない数字を引用つきで主張しない
- 固有名詞・略称は初出で「何のシステムか」を一言添える（例: 木星探査機 JUICE の高分解能カメラ JANUS）
- 段落構成は「一般論・先行研究（引用）→ 本研究固有の主張」の順
- 専門用語の初出は状況説明とセットで（例: 「初期捕捉走査の中心 (scan center)」）
- 「なぜ着目するか」はサイズと予測可能性の2点で説明する型を使う
- 差分の主張はニッチに見えないよう一般形で書き、具体の差分表は§2に回す


#### 関連研究と本稿の位置づけ
- 「これらの研究は、初期指向誤差のうち既知・予測可能な成分を事前に補正することの有効性を示しているが、衛星構体の熱状態に基づいて STT-LCT 相対LOSを予測し、scan center に反映する枠組みは対象とされていない。」：
  - STT-LCT相対LOSを... 以下が、前半の文に対して細かすぎる。これは今回の具体的な手法を含んでしまっているから、もう少し一般的なことを言いたい。例えば、「熱ひずみによる指向誤差をfeedforward的に予測して補償する」とか。どれくらい一般的なことまで言えるかは考えてほしい。
- 「2.3.地球観測系の熱変形LOS補正」について：
  - HuやLiの提案している手法にもう少しだけ触れてもいいかも（なんか、熱ひずみ予測とか言いながら結局全部機械学習でなんとかするやつとかなかったけ。あとfourierとか。wordだけでも）
  - 「地球観測とかでは式で熱ひずみfeedforwardを扱う研究はあるけど、光通信にはない」というのは、それはそう
- 「JANUS の研究と本研究の差分を表 表 1 にまとめる」
  - めちゃくちゃ細かいが、typstの記法的に、引用すると「表」は勝手についてくる。英語にするとどうなるかは不明なので、英語はまたあとで注意
- 「JANUS の研究と本研究の差分を表 表 1 にまとめる。対象システムは、JANUS が単一光学ヘッド内部のLOSであるのに対し、本研究は衛星バス上に離隔して搭載されたSTTとLCTの相対LOSである。補正量も、校正状態からのLOS/AKE変化ではなく、粗捕捉開始時の scan-center error である。温度入力は、光学ヘッド内の壁間温度差ではなく、衛星バスの太陽面と反対面のパネル温度差と運用フラグである。モデルは、校正基準を前提とする原点通過の比例モデルではなく、未校正のケース差や運用状態依存のDCを明示的に扱う階層モデルである。そして評価出力は、LOSとその不確かさではなく、LOS残差に加えて粗捕捉の捕捉時間と捕捉成功率である。」
  - うーん、言ってる事は合っているんだけど、やっぱ俺の研究側の言葉がちょっとspecific過ぎて、差分が伝わりにくい or すげーニッチを攻めている ように見えるんだよな。ここは全体的に表現改善したい
    - "AKE" という表現をこちらの論文でも使うべきか？平易な言い方ないかな（これ一般的なwordなの？）
    - 
    - 「原点通過の比例モデル」も、原点通貨がポイントというより、地上校正時からのオフセットという点が大事なんじゃないの？あーまああでもそれは書いてくれているのか。
    - あと、結構大事な差分として、こちらはより一般的な軌道・熱環境に対しても解析していてそれに対応したモデルも作っている、という点があると思うけど、この点どう思う？
    - 
  - あと、最後の評価出力に関して、今回の研究でもLOS残差は見てるからその点ではJANUSと同じだと思うけど、どう思う？（君って先行研究のPDF読めてる？）
- 先行研究の表に関して、もちろんメインで比較すべきはJANUSだが、他の先行研究についても（全部でなくとも）何個か並べると、表から一発で研究の位置づけわかるからいいと思う。
  - 改めて、先行研究のフォルダ内のPDFをKimiに読んでもらい、再確認。
- 「なお、JANUS には地上試験による検証があるのに対し、本研究の評価は現時点では数値解析に基づくものであり、実験的妥当性の面で優れていると主張するものではない。」 
...
- 「熱LOSを truth とみなし」
  - 一応真値と書いておいて
- 支配軸（θ_dom）の説明ってこれより前にあるんだっけ？ないなら説明追加して
- 支配軸でない軸の処理（確か定数？）も追加しておいて（短めでいいので）
- b_case = の式の説明のところで、発熱コンポが増えたらその分パラメータも増えることを説明しておいて
- 式の引用も「式 式3」みたいになってるから図、表と同様に修正しておいて
- 「1213COLD」ではなくCOLDでよいよ
  - てか表4にも1213COLDという表現があるが、これは通じるのか？
- 「標準の 1213COLD・既定被覆ケースでは...、固定10係数で熱LOS主成分を1–2桁低減できることを主結果とする。」：言い回しを既に修正した解析ケース行列の表に合わせて。


#### PAT初期捕捉評価
- 図8のPAT評価の流れの中に"spiral scan"とあるが、これは今回のスキャンの形式（矩形スキャン）に対応してる？まあ言い方の問題か
- スライドに、スパイラルスキャンしている図があるけど、これは追加しなくてもええか
  - もしかしたら序論とか序盤にあったらイメージしやすいのかなとか思ったけど。
  - まあ、スライドにあれば十分な図なのかなあれは
- 表7のここ「全走査で最坏 72.9 s」　漢字が日本語じゃない
- 表8見てて思ったんだけど、静的バイアスこんなに結果よかったっけ？これじゃあb caseモデルとあんまり変わらない気もする
  - これあれか、スキャンの条件変えて緩くしたからこうなっているのか。
  - うーん、それでもこんなに改善するんだっけ？


#### 軌道上の残差更新

- 「捕捉後に観測される支配軸上の残差を」
  - に続く残差rの式について、新しい文字が登場している気がするけど、前までの文字との整合性、新出文字の説明大丈夫？
- これ手法に関わる部分だけど、∂bって要るの？フーリエのc0に吸収されていない？
  - あーこれあれか、フーリエとは違う枠組みなのね。静的バイアス補正に近い形か
  - これもいるのかなあ
- 残差をフーリエでfitする手法を導入した理由を書きたい
  - たしか、非熱誤差においては軌道予測誤差が大きく占めていた点、軌道予測誤差の影響がSTT姿勢座標系の回転の影響から、sinカーブっぽく変化するからフーリエが効く、という話？
- 「いきなり fourier」
  - 日本語だとちょっと不自然だね。もう少しかっちょいいネーミングで（英語でもいいよ）
