...
from pat_acquisition.models.sunface_compo_local_los.features import (  # noqa: E402
    SUN_FACE_OPPOSITE,
...
from pat_acquisition.models.sunface_compo_local_los.summarize_coefficients import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate sunface+local-attach model: "
            "LOS ~ b + a*(T_sun-T_opp) + d_p*(T_prop-T_PY) + d_c*(T_pcdu-T_MY)."
        )
    )
    parser.add_argument("--dataset", type=Path, default=DEFAULT_DATASET)
    parser.add_argument("--cases", help="Case numbers, e.g. 4,13,14,15.")
...
def validate_one_case(
    *,
    ...
    x_all, feature_names, features_df, dominant_axis = build_compo_local_features(
    ...
    axis_model = train_compo_local_axis_model(
        x_train=x_train,
    ...
    pred_model = predict_compo_local_case(
        x=x_all,
    ...
    model_name = f"compo_local_{sun_face.lower()}_correction"
    ...
    predictions = pd.DataFrame(
        {
            ...
            "t_prop_attach_minus_py_center_c": features_df[
                "t_prop_attach_minus_py_center_c"
            ].to_numpy(dtype=float),
            "t_pcdu_attach_minus_my_center_c": features_df[
            ...
            "dtheta_x_pred_compo_local_urad": pred_model[:, 0],
            "dtheta_y_pred_compo_local_urad": pred_model[:, 1],
        }
    )
...
def main() -> None:
    ...
    config = CompoLocalFeatureConfig(ridge_lam=args.ridge_lam)
...
def plot_within_case(
    ...
    model_label = f"compo_local_{sun_face.lower()}"
    ...
    axes[0].plot(
        ...
        color="#1f77b4",
        label=f"T_{sun_face}-T_{opposite_face}" if opposite_face else "ΔT",
    )
    axes[0].plot(
        t_min,
        features_df["t_prop_attach_minus_py_center_c"],
        color="#ff7f0e",
        linestyle="--",
        label="T_prop - T_PY_center",
    )
    axes[0].plot(
        t_min,
        features_df["t_pcdu_attach_minus_my_center_c"],
        color="#2ca02c",
        linestyle=":",
        label="T_pcdu - T_MY_center",
    )
    ...
    axes[1].plot(t_min, pred_model[:, dom_idx], "--", label=f"compo_local pred {dom_name}")
    ...
    axes[2].plot(
        t_min,
        pred_model[:, other_idx],
        "--",
        label=f"compo_local pred {other_name} (static)",
    )
    ...
    err_model = np.linalg.norm(y_true - pred_model, axis=1)
    ...
    fig.suptitle(f"{case_tag} within-case compo_local: dT + local attach -> LOS {dom_name}")
    fig.tight_layout()
