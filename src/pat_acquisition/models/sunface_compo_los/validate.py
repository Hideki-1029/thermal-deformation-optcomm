...
from pat_acquisition.models.sunface_compo_los.dataset import (  # noqa: E402
    DEFAULT_DATASET,
    DEFAULT_OUTPUT_ROOT,
    list_numbered_cases,
    load_case_frame,
...
from pat_acquisition.models.sunface_compo_los.features import (  # noqa: E402
    SUN_FACE_OPPOSITE,
    CompoFeatureConfig,
...
from pat_acquisition.models.sunface_compo_los.summarize_coefficients import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cases",
        help="Case numbers, e.g. 4,5,6 or 4-6 (0-padding optional).",
    )
    parser.add_argument(
        "--case",
        default=None,
...
def validate_one_case(
    *,
    ...
    x_all, feature_names, features_df, dominant_axis = build_compo_features(
    ...
    model_name = f"compo_{sun_face.lower()}_correction"
    ...
    predictions = pd.DataFrame(
        {
            ...
            "t_opposite_c": features_df["t_opposite_c"].to_numpy(dtype=float),
            "t_sunface_minus_opposite_c": features_df[
                "t_sunface_minus_opposite_c"
            ].to_numpy(dtype=float),
            "t_prop_attach_c": features_df["t_prop_attach_c"].to_numpy(dtype=float),
            "t_pcdu_attach_c": features_df["t_pcdu_attach_c"].to_numpy(dtype=float),
            "t_prop_attach_minus_ref_c": features_df[
                "t_prop_attach_minus_ref_c"
            ].to_numpy(dtype=float),
            "t_pcdu_attach_minus_ref_c": features_df[
            ...
            "dtheta_x_pred_compo_urad": pred_compo[:, 0],
            "dtheta_y_pred_compo_urad": pred_compo[:, 1],
        }
    )
    ...
    predictions_path = case_output_dir / f"{case_tag}_{FILE_STEM}_predictions.csv"
    metrics_path = case_output_dir / f"{case_tag}_{FILE_STEM}_metrics.csv"
    coef_path = case_output_dir / f"{case_tag}_{FILE_STEM}_coefficients.csv"
    plot_path = case_output_dir / f"{case_tag}_{FILE_STEM}_true_vs_pred.png"
...
def main() -> None:
    ...
    config = CompoFeatureConfig(ridge_lam=args.ridge_lam, t_ref_c=args.t_ref_c)
...
def plot_within_case(
    ...
    model_label = f"compo_{sun_face.lower()}"
    ...
    axes[0].plot(
        t_min,
        features_df["t_sunface_minus_opposite_c"],
        color="#1f77b4",
        label=(
            f"T_{sun_face}-T_{opposite_face}"
            if opposite_face
            else "ΔT sun-opp"
        ),
    )
    axes[0].plot(
        t_min,
        features_df["t_prop_attach_minus_ref_c"],
        color="#ff7f0e",
        linestyle="--",
        label="T_prop - T_ref",
    )
    axes[0].plot(
        t_min,
        features_df["t_pcdu_attach_minus_ref_c"],
        color="#2ca02c",
        linestyle=":",
        label="T_pcdu - T_ref",
    )
    ...
    axes[1].plot(t_min, pred_compo[:, dom_idx], "--", label=f"compo pred {dom_name}")
    ...
    axes[2].plot(
        t_min,
        pred_compo[:, other_idx],
        "--",
        label=f"compo pred {other_name} (static)",
    )
    ...
    axes[3].plot(t_min, features_df["t_prop_attach_c"], label="T_prop_attach")
    axes[3].plot(t_min, features_df["t_pcdu_attach_c"], label="T_pcdu_attach")
    axes[3].axvline(train_end_min, color="k", linestyle=":", alpha=0.7)
    axes[3].set_ylabel("T attach [C]")
    axes[3].grid(True, alpha=0.3)
    axes[3].legend(fontsize=8)

    err_no = np.linalg.norm(y_true - pred_no, axis=1)
    err_static = np.linalg.norm(y_true - pred_static, axis=1)
    err_model = np.linalg.norm(y_true - pred_compo, axis=1)
    axes[4].plot(t_min, err_no, label="no_correction")
    axes[4].plot(t_min, err_static, label="static_bias")
    axes[4].plot(t_min, err_model, label=model_label)
    axes[4].axvline(train_end_min, color="k", linestyle=":", alpha=0.7)
    axes[4].set_ylabel("error norm [urad]")
    axes[4].set_xlabel("Time [min]")
    axes[4].grid(True, alpha=0.3)
    axes[4].legend(fontsize=8)

    title = (
        f"{case_tag} within-case compo: "
        f"dT + PROP/PCDU attach -> LOS {dom_name}"
    )
    fig.suptitle(title)
    fig.tight_layout()
