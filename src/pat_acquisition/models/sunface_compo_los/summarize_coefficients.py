"""
Aggregate per-case compo Ridge coefficients into a cross-case comparison CSV.
"""
...
DEFAULT_COMPARISON_CSV = DEFAULT_OUTPUT_ROOT / "compo_coefficients_comparison.csv"
DEFAULT_COMPARISON_DISPLAY_CSV = (
    DEFAULT_OUTPUT_ROOT / "compo_coefficients_comparison_display.csv"
)
...
COEF_COLUMNS = {
    ...
    "t_prop_attach_minus_ref_c": "coef_t_prop_attach_minus_ref_c_urad",
    "t_pcdu_attach_minus_ref_c": "coef_t_pcdu_attach_minus_ref_c_urad",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build cross-case sunface-compo coefficient comparison CSV."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=DEFAULT_OUTPUT_ROOT,
        help="compo output root containing case*_within_case/ folders.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def _read_test_metrics(case_dir: Path, case_tag: str, dominant_axis: str) -> dict[str, float]:
    ...
    model_rows = metrics_df[
        ...
        & (metrics_df["model"].astype(str).str.startswith("compo_"))
    ]
    if model_rows.empty:
        return {}
