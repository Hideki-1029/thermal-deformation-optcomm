"""
Within-case validation: LOS ~ b + a * (T_sunface - T_opposite).

Sun direction is treated as known. Evaluation is within-case
(first orbit train, remaining orbits test).
"""
...
from pat_acquisition.models.sunface_deltaT_los.dataset import (  # noqa: E402
    DEFAULT_DATASET,
    DEFAULT_OUTPUT_ROOT,
    list_numbered_cases,
    load_case_frame,
...
from pat_acquisition.models.sunface_deltaT_los.features import (  # noqa: E402
    SUN_FACE_OPPOSITE,
    DeltaTFeatureConfig,
...
from pat_acquisition.models.sunface_deltaT_los.summarize_coefficients import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate minimal sunface ΔT model: "
            "LOS ~ b + a*(T_sun - T_opp) (within-case)."
        )
    )
...
def validate_one_case(
    *,
    ...
    x_all, feature_names, features_df, dominant_axis = build_deltaT_features(
    ...
    model_name = f"deltaT_{sun_face.lower()}_correction"
    ...
    predictions = pd.DataFrame(
        {
            ...
            "dtheta_x_pred_deltaT_urad": pred_deltaT[:, 0],
            "dtheta_y_pred_deltaT_urad": pred_deltaT[:, 1],
        }
    )
    ...
    plot_within_case(
        ...
        features_df["t_sunface_minus_opposite_c"].to_numpy(dtype=float),
        train_mask,
    )
...
def main() -> None:
    ...
    config = DeltaTFeatureConfig(ridge_lam=args.ridge_lam)
...
def plot_within_case(
    ...
    axes[0].plot(
        ...
        linestyle="--",
        label=(
            f"T_{sun_face} - T_{opposite_face} (feature)"
            if opposite_face
            else "ΔT (feature)"
        ),
    )
    ...
    axes[1].plot(t_min, pred_deltaT[:, dom_idx], "--", label=f"deltaT pred {dom_name}")
    ...
    axes[2].plot(
        t_min,
        pred_deltaT[:, other_idx],
        "--",
        label=f"deltaT pred {other_name} (static)",
    )
    ...
    err_model = np.linalg.norm(y_true - pred_deltaT, axis=1)
    axes[3].plot(t_min, err_no, label="no_correction")
    axes[3].plot(t_min, err_static, label="static_bias")
    axes[3].plot(t_min, err_model, label=model_label)
    ...
    title = (
        f"{case_tag} within-case deltaT: "
        f"T_{sun_face}-T_{opposite_face} -> LOS {dom_name}"
        if opposite_face
        else f"{case_tag} within-case deltaT -> LOS {dom_name}"
    )
    fig.suptitle(title)
    fig.tight_layout()
